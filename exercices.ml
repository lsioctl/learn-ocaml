let rec facto x = match x with 0 -> 1 | n -> n * facto (n - 1)

let rec facto2 x = if x < 1 then 1 else x * facto2 (x - 1)

let rec tail = function [] -> None | [x] -> Some x | _ :: t -> tail t
...
let rec reverse = function [] -> [] | [x] -> [x] | h :: t -> reverse t @ [h]
...
let rec deduplicate_debug = function
  ...
  | l ->
      let _ = print_string "bouhhh" in
      l

let rec deduplicate = function
  | a :: (b :: _ as t) ->
      if a = b then deduplicate t else a :: deduplicate t
  | list_of_one_elt ->
      list_of_one_elt

let stack_push h l = h :: l

let stack_pop = function [] -> (None, []) | h :: t -> (Some h, t)

(** Careful, adding an element at the end of a list is anti pattern as it
    imposes a full copy see:
    ...
    the end of a list is an anti-pattern (aka a "snoc list" or a Schlemiel the
    Painter algorithm). Adding an element at the end of a list requires a full
    copy of the list. Adding an element at the front of the list requires
    allocating a single cell—the tail of the new list can just point to the old
    list.
...
let encode2 list =
  let rec aux count acc = function
    | [] ->
        [] (* original list empty*)
    | [x] ->
        if count = 0 then One x :: acc else Many (count + 1, x) :: acc
    | a :: (b :: _ as t) ->
        if a = b then
...
let encode3 list =
  let create_rle cnt elem =
    if cnt = 0 then One elem else Many (cnt + 1, elem)
  in
  let rec aux count acc = function
    | [] ->
        [] (* original list empty*)
    | [x] ->
        create_rle count x :: acc
    | a :: (b :: _ as t) ->
