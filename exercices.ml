...
let pack list =
  let rec aux current acc = function
    ...
    | [ x ] -> acc @ [ current @ [ x ] ]
    | a :: (b :: _ as t) ->
      if a = b then aux (current @ [ a ]) acc t else aux [] (acc @ [ current @ [ a ] ]) t
  in
  aux [] [] list
;;
  ...
  let rec aux count acc = function
    ...
    | a :: (b :: _ as t) ->
      ...
      then (
        ...
        if count = 0 then aux 0 (One a :: acc) t else aux 0 (Many (count + 1, a) :: acc) t)
  in
  List.rev (aux 0 [] list)
;;

let encode3 list =
  let create_rle cnt elem = if cnt = 0 then One elem else Many (cnt + 1, elem) in
  let rec aux count acc = function
    | [] -> [] (* original list empty*)
    | [ x ] -> create_rle count x :: acc
    | a :: (b :: _ as t) ->
      if a = b then aux (count + 1) acc t else aux 0 (create_rle count a :: acc) t
  in
  List.rev (aux 0 [] list)
;;
